...
import {
    ...
    MessageSquare, CheckCircle, CornerDownLeft
} from 'lucide-react';
...
export default function Verify() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [{ data: rejected }, { data: pending }, { data: returned }] = await Promise.all([
                supabase
    ...
    const tabs = [
        ...
        { id: 'returned', label: 'Returned by Head',  count: returnedRequests.length, icon: <CornerDownLeft size={14} /> },
    ...
    const EmptyState = ({ message }) => (
        <div className="text-center py-24 bg-white rounded-[4rem] border-2 border-dashed border-slate-100">
            ...
            <p className="text-slate-400 font-black uppercase text-xs tracking-[0.2em]">{message}</p>
        </div>
    );
        ...
        <div className="max-w-6xl mx-auto space-y-10">
            ...
            <div className="flex flex-col gap-2">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">
                    Overdue Queue · Returned by Head · Rejected Manager
                </p>
            </div>
                    ...
                    {activeTab === 'overdue' && (
                        <div className="space-y-4">
                            {overdueRequests.length === 0 ? (
                                <EmptyState message="No overdue requests" />
                            ) : (
                    ...
                    {activeTab === 'returned' && (
                        <div className="space-y-4">
                            {returnedRequests.length === 0 ? (
                                ...
                                <>
                                    {/* Info banner */}
                                    <div className="flex items-start gap-3 p-5 bg-amber-50 border border-amber-200 rounded-3xl">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] font-bold text-amber-600 mt-0.5 leading-relaxed">
                                                The Head Registrar has returned these applications. The uploaded files have been cleared.
                                                Re-open the application in the main queue, upload the correct documents, and re-submit for verification.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {returnedRequests.map(req => (
                                        <div
                                            key={req.application_id}
                                            className="bg-white p-8 rounded-[2.5rem] border border-amber-200 border-l-4 border-l-amber-400 shadow-sm"
                                        >
                                            <div className="flex flex-wrap items-center justify-between gap-6">
                                                <div className="flex items-center gap-6">
                                                    <div className="w-14 h-14 bg-amber-50 rounded-3xl flex items-center justify-center">
                                                        <CornerDownLeft size={24} className="text-amber-400" />
                                                    </div>
                                                    <div>
                                                        <h3 className="font-black text-slate-900 uppercase tracking-tight text-lg">
                                                            {req.student_name}
                                                        ...
                                                        {req.rejection_reason && (
                                                            <p className="text-[10px] text-amber-500 font-bold mt-2 flex items-center gap-1.5">
                                                                <AlertCircle size={10} />
                                                                {req.rejection_reason}
                                                            </p>
                                                        )}
                                                ...
                                                <span className="flex items-center gap-2 px-4 py-2.5 rounded-2xl border text-[10px] font-black uppercase bg-amber-100 text-amber-600 border-amber-200">
